...
import { Object, Animator, AnimatorClipInfo, Resources, AnimationClip, WaitForSeconds, AnimatorOverrideController, Mathf, Physics, Vector3, GameObject } from 'UnityEngine';
...
export default class PlayerSync extends ZepetoScriptBehaviour {
    ...
    private OnChangedPlayer() {
        ...
        // console.log(` ${WorldService.userName}'s OTHER : ${animationParam.IsSit} ${this.m_animator.GetBool("isSit")}`);
        ...
        if (animationParam.State == CharacterState.Gesture && this.UseZepetoGestureAPI || this.GetAnimationClipFromResources ) { 
            ...
            if (clipInfo[0] == undefined || clipInfo[0] == null) {
                return;
            }
            // if (clipInfo[0].clip.name == gestureName) {
            //     console.log(" ========================================================= clipInfo[0].clip : "+clipInfo[0].clip);
            //     return;
            // }
            // if (clipInfo[0].clip.name == gestureName) {
            //     console.log(" ========================================================= clipInfo[0].clip.name : "+gestureName);
            //     return;
            // }
    ...
    private* SendLocalPlayer(tick: number) {
        ...
        SyncIndexManager.Score = this.player.score;
        ...
        const recorderManager = GameObject.FindObjectOfType<RecorderManager>();
        if(recorderManager) {
        ...
        while (true) {
            ...
            if(state != CharacterState.Idle || pastIdleCount < pastIdleCountMax) {
                ...
                animationParam.Add("IsSit", this.m_animator.GetBool("isSit"));
            ...
            if(state == CharacterState.Idle && !this.m_animator.GetBool("isSit"))             //Send 10 more frames even if stopped
                pastIdleCount++;
            else
                pastIdleCount = 0;
