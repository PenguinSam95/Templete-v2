...
export default class MultiplayManager extends ZepetoScriptBehaviour {
    ...
    @SerializeField() private ghost:GameObject;
    ...
    private GetInstantiate(){
        ...
        this.room.AddMessageHandler(MESSAGE.Instantiate, (message:InstantiateObj) => {
            let prefabObj = Resources.Load(message.prefabName) as GameObject;
            if(message.prefabName == "Ghost(TransformSync)" && this.ghost != null)
    ...
    private *SendPing(ping:number){
        ...
        const wait = new WaitForSeconds(ping);
        while(true) {
            if(!this.bPaused) {
